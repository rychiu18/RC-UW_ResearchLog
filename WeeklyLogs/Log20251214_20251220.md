...
* Interaction index ($\mu$) can describe dominant processes between ocean and river influence 
...
* Interaction index to characterize flow interaction ($\mu$)
	* Ratio index used in Juarez et al. (2022) to quantify interaction of river processes with ocean processes for every 12 hour tidal cycle (Fig. 5)
	* $\mu$ = min(u_mouth)/max(u_upstream)
	* $\mu$ <0 - ocean dominates
	* $\mu$ >0 - river dominates
	* We can see when comparing both the velocity at the head and Montesano with the mouth that river influence dominates just after December 9th, where water levels become elevated AND damped

![IndexRatio](../Figures/121825meeting/Dec2015_interactionindex.png)<br>
Figure 5: Water levels at Westport and Montesano (top). Interaction index for mouth vs. head velocities, and for mouth vs. Montesano velocities (bottom). 


### 2) Dec2025 storm post-storm vs. forecast
* Ran TMD (forecast), TMDW (forecast), and TMD (Fig. 6)
...
Figure 6: Comparisons between TMD forecast, TMDW forecast, TMD post-storm model, and measured water levels for the December 2025 storm


## Next steps:
* Run TMDW post-storm model for December 2025 storm
* December 2015 analysis
	* I hypothesize that the previous analysis of MET and discharge influence might be better represented if I run:
		* Tides only with base discharge (September 2015, semi-low flow) to more accurately model influence of increased discharge due to Dec. 2015 storm
		* Tides+MET with base discharge (September 2015, semi-low flow) to more accurately model influence of increased discharge due to Dec. 2015 storm
* Look at the Willapa field data from the Nov-Dec deployment